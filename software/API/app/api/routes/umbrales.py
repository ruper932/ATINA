...
async def create_parametro(obj_in: ParametroUmbralCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def read_parametros(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def update_parametro(id: int, obj_in: ParametroUmbralUpdate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    obj = await crud.parametro_umbral.get(db=db, id=id)
    if not obj: raise HTTPException(status_code=404, detail="Parámetro no encontrado")
...
async def create_configuracion(obj_in: ConfiguracionUmbralCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Inyectamos el usuario actual como responsable de la actualización
    obj_in.actualizado_por = current_user.id
...
async def read_configuraciones(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def update_configuracion(id: int, obj_in: ConfiguracionUmbralUpdate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    obj = await crud.configuracion_umbral.get(db=db, id=id)
    if not obj: raise HTTPException(status_code=404, detail="Configuración no encontrada")
    obj_in.actualizado_por = current_user.id
